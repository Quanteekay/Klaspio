...
import { getLessonsByTeacher, toCalendarDay } from "@/src/services/lessonsApi";
...
export default function TeacherCalendar() {
  ...
  const fetchLessons = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getLessonsByTeacher(uid);
      setLessons(data);
    } catch {
            ...
            visibleLessons.map((lesson) => (
              ...
              >
                ...
                <Text style={[styles.lessonMeta, { color: t.colors.textSecondary }]}>
                  ...
                  {` • ${lesson.studentIds.length} uczniów`}
                </Text>
              </Pressable>
            ))
          )}
...
const styles = StyleSheet.create({
  content: { padding: 15, paddingBottom: floatingTabBar.contentBottomPadding },
